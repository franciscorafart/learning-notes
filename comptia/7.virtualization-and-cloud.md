...
- Virtual Machine Sprawl => Config issue were deployment of VMs is not properly authorized or monitored. It can lead to Rougue VMs => Installed without authorization
...
- Cloud solution provide compute resources over a network (servers, databases, containersstorage, apps)
...
- Virtualization server can be seployed on-prem or in the cloud.
- Virtual Desktops easier to maintain and troubleshoot - Desktop as a Service (DaaS)
- Company can oofload entire IT infrastructure to a third-party service.
...
- Northbound API (to application layer) and Southbound API (To infrastructure laeyr)
