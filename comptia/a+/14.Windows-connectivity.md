...
- Share tab on folders properties => customize permissions, liit simultaneous connections, 
- Windows automatically creates hidden administrative shares => Root folders on ocal drives and system folders. Remain password protected even if password protection disabled.

## Network browsing and Mapping Drives

- Shares are listed under "file server" omputer on the Network object in file explorer, bith in workgroup and domain networks.
- PCs identified by hostname

### Mapped drives
- mapped drive: shared network folder with drive letter
- Right click and select `Map Netowork Drive` => Check reconnect and sign-in if you want to keep it shared. Select `Disconnect` to remove shared drive.
