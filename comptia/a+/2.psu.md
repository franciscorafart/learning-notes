...
- Gaming PCs can require up to 500, specially to Graphics cards and CPUs
...
A PSU will have a number of other Molex (4-pin) and SATA power cables (15-pin), and 4/6/8 pin connectors for CPU and PCIe cards.
...
- Chassis has several drive bays where we can install different form factor drives
- A disk is usually attached to the drive bay using a `Caddy`. Drive is screwed to the caddy and the caddy is screwed to the drive bay.
...
### RAID 5: Strirping with Distributed Party
...
Synchronous Dynamic (SDRAM) => Synchronized to the motherboard's clock.
...
## Install CPUS
 - Ground ESD device to inside of the case
...
### CPU features
- SMT (Simultaneous Multithreaded) makes it possible to make execution more efficient, even though most programs are single-threaded.
- SMP Symmetric Multi processing: A SMP OS can allow for running application processes whenever CPU is available. Costly, more common in servers and high-end workstations.
