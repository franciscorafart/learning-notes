...
Local Mail Client (Sending email)
    Stores sent email in local IMAP server (Port 993)
...
### Single Sign On (SSO)
Authentication processes that llow users to access all the network and authorization for applications running on a server.
...
- Terminal TTY device refers to a teletype device used in the old days to 
...
- Also Open source server products, as well as clients for MacOs, Linux, Android, iOS
...
- A Log containes 1. PRI code 2. Header containing timestamp and host name and 3. Message. Format of message application dependent.
...
- Electronic device design for a specific function
...
- Embedded system network is referred to as Operational Technology (OT) network, different from IT network
...
1. Establish scopr of issue. If it's for multiple hosts, then the issue might be in the DHCP server
