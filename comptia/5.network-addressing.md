...
3. Network Layer / Internet => Packet (Network Header + Transport Header + Data) (IP)
4. Link => Frame (Frame + Network Header + Transport Header + Data + Frame) . Frames are different for ethernet and wifi (Router)
    4.2 Phsysical => Bits
...
1. Network Address Translation (NAT) -Router: Through a router configured with a single or block of valid IP addresses: Router uses Network Address Tranlation (NAT) to turn private address into public one.
...
- Typically assinged to services with a speciic functionality like router or application server
...
- IPV6 doesn't need to be configured with a default gateway => It used Neighbot Discovery (ND) protocol
...
## Protocols and Ports
Link layer => Allows local frame communication via MAC addresses
...
As a connection-oriented protocol, TCP/IP mitigatesthis problem by:
...
3. Receiver can ACK acknowledge a package has been received
...
- TFTP (Trivial File Transfer Protocol) => Used by network devices to get configuration file, uses own achnowledgement process so it doesn't require TCP.
...
- Fully Qualified Domain Name (FQDM): Host name can be combined with a domain name and suffix to avoi duplication
...
A: IPV4 address
AAAA: IPV6 address
MX: Mail Exchanger => To identify email services so that other servers can send messages to it. Host name in the MX record must jave an IPV4 or IPV6 address
...
- This can reduce performance if hundreds or thousands of ports are placed in the same boradcast domain
